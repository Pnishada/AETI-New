...
const AboutPage: React.FC = () => {
  ...
  return (
    <div className="bg-gradient-to-r from-blue-300 via-blue-500 to-blue-300 text-white">
      {/* Hero Section */}
      <section className="max-w-7xl mx-auto grid md:grid-cols-2 gap-6 px-6 py-12 items-center">
        {/* Left: Text */}
        <motion.div
      ...
      <section className="bg-white py-12">
        <div className="max-w-6xl mx-auto px-6 text-center">
          ...
          <ul className="text-left mt-4 space-y-2 max-w-md mx-auto text-gray-900">
            {courses.map((course, index) => (
              <li
                key={index}
                onClick={() => setSelected(course)}
                className={`cursor-pointer transition ${
                  selected === course ? "font-bold text-blue-700" : "hover:font-bold"
                }`}
              >
                • {course}
              </li>
      ...
      <section className="bg-gray-200 text-black py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
          {/* Mission Card */}
          <Card className="relative overflow-hidden shadow-2xl rounded-2xl hover:shadow-3xl transition-shadow duration-500 transform hover:-translate-y-1 hover:scale-105 bg-gradient-to-r from-blue-200 to-blue-400 text-black">
            <div className="absolute inset-0 bg-gradient-to-tr from-blue-300 to-blue-200 opacity-20 animate-pulse"></div>
      ...
      <section className="py-12 bg-white">
        <div className="max-w-6xl mx-auto px-6 text-center">
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Why Choose AETI</h2>
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-8">
            {[{ icon: Building2, title: "Govt. Certified Training" },
              ...
              { icon: Users, title: "Overseas scholarships" }].map((item, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 20 }}
